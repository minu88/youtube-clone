...
const Head = () => {
  ...
  const [ suggestions, setSuggestions ] = useState([]);
   ...
   // useeffect to call API showing debouncing using setTimeOut()
   useEffect( () => {
    // APi Call with debouncing // make api call //but if the difference between two api calls is <200ms then decline the API call
  ...
  /*
  cache in the searchSlice
    {
      "iphone": ["ihone", "ihpone16"]l
    }
  */

  // toggle sidebar using dispatch
  ...
  return (
    <div className="grid grid-flow-col p-2 my-2 shadow-lg">
      ...
      <div className="col-span-10 px-10">
        ...
        />
        <button className="py-2 px-4 rounded-r-full border border-gray-400 bg-gray-300">
          🔎
        </button>
      </div>
      {/* search pop up */}
      { 
      showSearchQuery && 
      ( <div className="absolute top-16 left-[18.5rem] w-[26rem] shadow-lg rounded-lg bg-white ">
          <ul className=" ">
            {
              suggestions.map((name) => <li key={name} className="hover:bg-gray-200 px-12 py-2 cursor-pointer"> {name} </li>)
            }
          </ul>
        </div>
      )
